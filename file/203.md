...
下面代码展示了类在ES6中的使用。再次啰嗦一句，你可以将代码贴到[traceur](http://google.github.io/traceur-compiler/demo/repl.html)自己查看运行结果。
<div class="highlight">
<pre>//类的定义
class Animal {
	//ES6中新型构造器
...
<div class="highlight">
<pre>var people=['Wayou','John','Sherlock'];
...
<div class="highlight">
<pre>for (let i=0;i&lt;2;i++)console.log(i);//输出: 0,1
...
_注意，此功能google traceur并未实现，所以无法模拟调试,下面有些功能也是如此_
...
将不同功能的代码分别写在不同文件中，各模块只需导出公共接口部分，然后通过模块的导入的方式可以在其他地方使用。下面的例子来自[tutsplus](http://code.tutsplus.com/tutorials/eight-cool-features-coming-in-es6--net-33175):
<div class="highlight">
<pre>// point.js
module "point" {
    export class Point {
...
下方代码来自[es6feature](https://github.com/lukehoban/es6features)
<div class="highlight">
<pre>// Sets
var s = new Set();
...
正文代码来自[es6feature](https://github.com/lukehoban/es6features)
<div class="highlight">
<pre>// Weak Maps
var wm = new WeakMap();
wm.set(s, { extra: 42 });
...
以下例子借用自[这里](http://ariya.ofilabs.com/2013/07/es6-and-proxy.html)。`
`
<div class="highlight">
<pre>//定义被侦听的目标对象
...
以下例子来自[es6features](https://github.com/lukehoban/es6features)
<div class="highlight">
<pre>(function() {
...
Promises是处理异步操作的一种模式，之前在很多三方库中有实现，比如jQuery的[deferred](http://api.jquery.com/category/deferred-object/) 对象。当你发起一个异步请求，并绑定了.when(), .done()等事件处理程序时，其实就是在应用promise模式。
<div class="highlight">
<pre>//创建promise
